...
export const ZONES_QUIZZES: Record<string, QuizLevel[]> = {
  ...
  sql: [
    ...
    {
      level: 'sql-delete-truncate-drop',
      questions: [
        {
          question: "You want to remove all test orders created before 2024 from the orders table, but you're not 100% sure of the count. What is the safest approach?",
          options: [
            { id: 'a', text: "Run DELETE FROM orders WHERE created_at < '2024-01-01'; immediately.", isCorrect: false },
            { id: 'b', text: "Run SELECT COUNT(*) first to see how many rows match, wrap the DELETE in BEGIN/COMMIT, verify ROW_COUNT() before committing.", isCorrect: true },
            { id: 'c', text: "Use TRUNCATE TABLE orders — it's faster and cleaner.", isCorrect: false },
            { id: 'd', text: "Use DROP TABLE orders and recreate it.", isCorrect: false },
          ],
          explanation: "Always do a dry-run SELECT COUNT(*) with the same WHERE clause to confirm the blast radius before deleting. Wrapping in a transaction (BEGIN/COMMIT) lets you check ROW_COUNT() and ROLLBACK if the numbers look wrong. TRUNCATE removes ALL rows with no WHERE clause and no rollback, which would wipe far more than intended."
        },
        {
          question: "After TRUNCATE TABLE test_orders, a developer inserts a new test order. What id value will the new row get?",
          options: [
            { id: 'a', text: "The next sequential id after the last row before TRUNCATE (e.g., 5001).", isCorrect: false },
            { id: 'b', text: "1, because TRUNCATE resets the AUTO_INCREMENT counter.", isCorrect: true },
            { id: 'c', text: "NULL, because TRUNCATE removes the id column definition.", isCorrect: false },
            { id: 'd', text: "A random id generated by the database.", isCorrect: false },
          ],
          explanation: "TRUNCATE resets the AUTO_INCREMENT counter to 1 (or the defined starting value). This is a key difference from DELETE — DELETE leaves the counter where it was, so the next row gets id 5001. This matters in QA when test cases hardcode expected id values."
        },
        {
          question: "You run DROP TABLE IF EXISTS staging_data. A colleague's test script is currently running and reading from that table. What happens?",
          options: [
            { id: 'a', text: "The DROP waits politely until the other query finishes, then executes.", isCorrect: false },
            { id: 'b', text: "The DROP acquires an exclusive lock, blocking the colleague's query, then permanently destroys the table.", isCorrect: true },
            { id: 'c', text: "The DROP only removes the rows, not the table structure.", isCorrect: false },
            { id: 'd', text: "IF EXISTS makes the DROP safe — it only drops tables that nobody is using.", isCorrect: false },
          ],
          explanation: "DROP TABLE acquires an exclusive metadata lock, which blocks all other operations on that table. Once the lock is acquired and DROP executes, the table is permanently gone — no rollback. IF EXISTS only prevents an error if the table doesn't exist; it doesn't make the operation safe for concurrent use."
        },
        {
          question: "You need to delete a customer with id=500, but the orders table has a foreign key referencing customers with ON DELETE RESTRICT. What must you do first?",
          options: [
            { id: 'a', text: "Use TRUNCATE on orders to clear the way, then DELETE the customer.", isCorrect: false },
            { id: 'b', text: "Delete or reassign the child rows in orders (and any other referencing tables) before deleting the customer.", isCorrect: true },
            { id: 'c', text: "Use DROP TABLE customers — it bypasses foreign key checks.", isCorrect: false },
            { id: 'd', text: "Nothing — RESTRICT means the delete is simply skipped without an error.", isCorrect: false },
          ],
          explanation: "ON DELETE RESTRICT blocks the parent row deletion while child rows still reference it. You must delete the child rows first (in correct dependency order), then delete the parent. TRUNCATE is not appropriate here — it removes ALL rows. DROP would destroy the table entirely. RESTRICT causes an error, not a silent skip."
        },
        {
          question: "You want to remove only users whose emails contain 'automation-bot' without affecting any other rows. Which command is correct?",
          options: [
            { id: 'a', text: "TRUNCATE TABLE users WHERE email LIKE '%automation-bot%';", isCorrect: false },
            { id: 'b', text: "DELETE FROM users WHERE email LIKE '%automation-bot%';", isCorrect: true },
            { id: 'c', text: "DROP TABLE users WHERE email LIKE '%automation-bot%';", isCorrect: false },
            { id: 'd', text: "REMOVE FROM users WHERE email LIKE '%automation-bot%';", isCorrect: false },
          ],
          explanation: "DELETE is the only one of the three that supports a WHERE clause for targeted removal. TRUNCATE removes ALL rows and doesn't accept WHERE. DROP removes the entire table structure and data. REMOVE is not a valid SQL command."
        },
      ]
    },

    {
      level: 'sql-insert-advanced',
      questions: [
        {
          question: "You want to copy all orders from 2025 into a table called test_orders_2025 that already exists with the same columns. Which statement does this in one shot?",
          options: [
            { id: 'a', text: "COPY orders TO test_orders_2025 WHERE YEAR(created_at) = 2025;", isCorrect: false },
            { id: 'b', text: "INSERT INTO test_orders_2025 SELECT * FROM orders WHERE YEAR(created_at) = 2025;", isCorrect: true },
            { id: 'c', text: "CREATE TABLE test_orders_2025 AS SELECT * FROM orders WHERE YEAR(created_at) = 2025;", isCorrect: false },
            { id: 'd', text: "INSERT INTO test_orders_2025 FROM orders WHERE YEAR(created_at) = 2025;", isCorrect: false },
          ],
          explanation: "INSERT INTO ... SELECT copies rows from the SELECT result directly into the target table. This requires the target table to already exist. CREATE TABLE AS SELECT would also work but creates a brand-new table — if test_orders_2025 already exists, it would error. COPY and the malformed INSERT FROM are not valid SQL syntax."
        },
        {
          question: "Your seed script runs every time the CI pipeline starts. It inserts reference data (roles, categories) that may already exist. Which INSERT variant prevents errors on duplicate runs without deleting existing data first?",
          options: [
            { id: 'a', text: "Plain INSERT — it will just add duplicates which is fine.", isCorrect: false },
            { id: 'b', text: "INSERT IGNORE — skips rows that violate UNIQUE constraints silently.", isCorrect: true },
            { id: 'c', text: "TRUNCATE then INSERT — clear everything and re-insert.", isCorrect: false },
            { id: 'd', text: "CREATE TABLE AS SELECT — recreate the table from scratch each time.", isCorrect: false },
          ],
          explanation: "INSERT IGNORE skips any row that would violate a UNIQUE or PRIMARY KEY constraint, making the script idempotent — safe to run multiple times without errors or duplicates. Plain INSERT would fail with a duplicate key error on the second run. TRUNCATE then INSERT is risky if other tables have FK references to this data."
        },
        {
          question: "After CREATE TABLE orders_backup AS SELECT * FROM orders WHERE status = 'completed', a developer tries to add a new completed order directly to orders_backup. They find there is no AUTO_INCREMENT on the id column. Why?",
          options: [
            { id: 'a', text: "CREATE TABLE AS SELECT copies all data and constraints including AUTO_INCREMENT.", isCorrect: false },
            { id: 'b', text: "CREATE TABLE AS SELECT copies column data types and values but does NOT copy PRIMARY KEY, constraints, or AUTO_INCREMENT.", isCorrect: true },
            { id: 'c', text: "The backup table has a different storage engine that doesn't support AUTO_INCREMENT.", isCorrect: false },
            { id: 'd', text: "AUTO_INCREMENT only works on the original table and cannot be transferred.", isCorrect: false },
          ],
          explanation: "CREATE TABLE AS SELECT is a 'data copy' operation — it reproduces column types and row data, but NOT the schema metadata: no PRIMARY KEY, no UNIQUE constraints, no indexes, no AUTO_INCREMENT, no foreign keys. After creating the backup, you must manually add these with ALTER TABLE if needed."
        },
        {
          question: "You run this statement: INSERT INTO page_views (slug, view_date, count) VALUES ('home', CURDATE(), 1) ON DUPLICATE KEY UPDATE count = count + 1; What happens if a row with the same slug + view_date already exists?",
          options: [
            { id: 'a', text: "The INSERT fails with a duplicate key error.", isCorrect: false },
            { id: 'b', text: "A second row is inserted with count = 1.", isCorrect: false },
            { id: 'c', text: "The existing row's count is incremented by 1 instead of inserting a new row.", isCorrect: true },
            { id: 'd', text: "The existing row is deleted and a new one with count = 1 is inserted.", isCorrect: false },
          ],
          explanation: "ON DUPLICATE KEY UPDATE is an upsert — if the INSERT would violate a UNIQUE or PRIMARY KEY constraint, it switches to an UPDATE on the conflicting row instead. This is perfect for tracking counters, last-seen timestamps, or any data that should be created on first touch and updated on subsequent ones."
        },
        {
          question: "Before running a full regression test suite, you want to ensure the test_orders table has exactly 100 rows of completed orders and nothing else. Which pattern achieves this safely?",
          options: [
            { id: 'a', text: "INSERT IGNORE INTO test_orders SELECT * FROM orders WHERE status = 'completed' LIMIT 100;", isCorrect: false },
            { id: 'b', text: "BEGIN; TRUNCATE TABLE test_orders; INSERT INTO test_orders SELECT * FROM orders WHERE status = 'completed' LIMIT 100; SELECT COUNT(*) FROM test_orders; COMMIT;", isCorrect: true },
            { id: 'c', text: "DROP TABLE test_orders; CREATE TABLE test_orders AS SELECT * FROM orders LIMIT 100;", isCorrect: false },
            { id: 'd', text: "DELETE FROM test_orders; INSERT INTO test_orders SELECT * FROM orders LIMIT 100;", isCorrect: false },
          ],
          explanation: "TRUNCATE inside a transaction gives you a clean slate, then INSERT fills it with exactly the data you want. The COUNT(*) check before COMMIT confirms the row count is exactly 100. INSERT IGNORE on a non-empty table would skip existing rows, leaving unknown state. DROP + CREATE loses constraints and doesn't guarantee count."
        },
      ]
    },
    ...
    {
      level: 'sql-error-handling',
      questions: [
        {
          question: "A stored procedure transfers funds between two accounts. The first UPDATE (debit) succeeds, but the second UPDATE (credit) hits a constraint violation. Without error handling, what is the result?",
          options: [
            { id: 'a', text: 'Both UPDATEs are automatically rolled back — MySQL always handles this.', isCorrect: false },
            { id: 'b', text: 'The debit is permanently applied but the credit never happens — money disappears from the system.', isCorrect: true },
            { id: 'c', text: 'The procedure retries the failed UPDATE automatically.', isCorrect: false },
            { id: 'd', text: 'MySQL queues the failed UPDATE and retries it when the constraint is resolved.', isCorrect: false },
          ],
          explanation: "Without a transaction and error handler, each statement auto-commits independently. The debit UPDATE commits and is permanent. The credit UPDATE then fails — but the debit is already done. This is exactly the data corruption scenario that DECLARE...HANDLER + transactions are designed to prevent."
        },
        {
          question: "In a stored procedure, you declare: DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN ROLLBACK; SET p_status = 'ERROR'; END; What does EXIT mean in this context?",
          options: [
            { id: 'a', text: 'The handler exits the entire database connection.', isCorrect: false },
            { id: 'b', text: 'After the handler executes, control exits the BEGIN...END block where the handler was declared.', isCorrect: true },
            { id: 'c', text: 'The handler runs and then continues execution from the line that caused the error.', isCorrect: false },
            { id: 'd', text: 'EXIT means the handler only fires once, then is deactivated.', isCorrect: false },
          ],
          explanation: "EXIT HANDLER fires when an error occurs, runs the handler body, then exits the BEGIN...END block — effectively ending that scope. This is the right choice for most error recovery patterns. CONTINUE HANDLER would resume execution on the line after the error, which can leave variables in an undefined state."
        },
        {
          question: "You want a stored procedure to enforce a business rule: orders cannot be placed for amounts less than ₹1. The amount column allows any DECIMAL value. Which SQL mechanism correctly raises a custom error when amount < 1?",
          options: [
            { id: 'a', text: "DECLARE HANDLER FOR 'amount_too_small' BEGIN ... END;", isCorrect: false },
            { id: 'b', text: "SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Amount must be at least ₹1';", isCorrect: true },
            { id: 'c', text: "THROW ERROR 'Amount must be at least ₹1';", isCorrect: false },
            { id: 'd', text: "RAISE EXCEPTION 'Amount must be at least ₹1';", isCorrect: false },
          ],
          explanation: "SIGNAL SQLSTATE '45000' is the MySQL standard for raising a user-defined exception. SQLSTATE '45000' means 'unhandled user-defined exception'. THROW and RAISE EXCEPTION are SQL Server and PostgreSQL syntax respectively, not valid in MySQL."
        },
        {
          question: "You are testing a stored procedure that uses GET DIAGNOSTICS CONDITION 1 ... MYSQL_ERRNO. You call the procedure with a duplicate email. The OUT parameter returns 'ERROR 1062: Duplicate entry...'. What does this tell you about the procedure's error handling?",
          options: [
            { id: 'a', text: 'The procedure crashed and the error bubbled up to the client.', isCorrect: false },
            { id: 'b', text: 'The error handler caught the exception, extracted the specific error code and message, and returned them via the OUT parameter — exactly as intended.', isCorrect: true },
            { id: 'c', text: 'The procedure ignored the error and continued executing.', isCorrect: false },
            { id: 'd', text: 'MySQL automatically added the GET DIAGNOSTICS call — the procedure did not write this.', isCorrect: false },
          ],
          explanation: "GET DIAGNOSTICS CONDITION 1 reads the details of the most recent error — error code (MYSQL_ERRNO) and message text (MESSAGE_TEXT). The handler catching this and returning it as a structured OUT parameter means the procedure is doing its job correctly: intercepting the error, extracting details, and communicating failure cleanly to the caller."
        },
        {
          question: "You want to test that a fund transfer procedure leaves account balances unchanged when the transfer fails. Which query provides the strongest assertion?",
          options: [
            { id: 'a', text: "SELECT balance FROM accounts WHERE id = 1;  -- check one account", isCorrect: false },
            { id: 'b', text: "SELECT SUM(balance) FROM accounts;  -- the total must be the same before and after any failed transfer", isCorrect: true },
            { id: 'c', text: "SELECT COUNT(*) FROM accounts;  -- verify no accounts were deleted", isCorrect: false },
            { id: 'd', text: "SHOW ERRORS;  -- check the MySQL error queue", isCorrect: false },
          ],
          explanation: "The strongest atomicity check is verifying that the system-wide total balance is unchanged. Even if individual balances shift by the same amount, SUM(balance) must equal the pre-test value after any failed transfer. Checking one account only validates half the transaction. COUNT(*) checks existence, not money. SHOW ERRORS only shows the last error, not data state."
        },
      ]
    },

    {
      level: 'sql-pivot-reporting',
      questions: [
        {
          question: "You want a single query that shows, for each product, how many orders are 'pending', 'completed', and 'cancelled' — all in separate columns. Which technique achieves this?",
          options: [
            { id: 'a', text: "Run three separate queries filtered by status and JOIN them together.", isCorrect: false },
            { id: 'b', text: "SELECT product_name, COUNT(CASE WHEN status='pending' THEN 1 END) AS pending, COUNT(CASE WHEN status='completed' THEN 1 END) AS completed, COUNT(CASE WHEN status='cancelled' THEN 1 END) AS cancelled FROM orders GROUP BY product_name;", isCorrect: true },
            { id: 'c', text: "SELECT product_name, status, COUNT(*) FROM orders GROUP BY product_name, status;", isCorrect: false },
            { id: 'd', text: "SELECT product_name, PIVOT(status) FROM orders;", isCorrect: false },
          ],
          explanation: "CASE inside COUNT is the MySQL way to pivot. Each CASE acts as a filter — only rows where the condition is true contribute a 1 to that column's COUNT. Non-matching rows return NULL, which COUNT ignores. The three separate queries + JOIN approach works but is verbose and hard to maintain. PIVOT is not MySQL syntax."
        },
        {
          question: "In a pivot query: SUM(CASE WHEN MONTH(order_date) = 1 THEN amount ELSE 0 END) AS Jan. Why is ELSE 0 used instead of ELSE NULL?",
          options: [
            { id: 'a', text: "ELSE NULL would cause an error in SUM.", isCorrect: false },
            { id: 'b', text: "ELSE 0 ensures non-January rows contribute 0 to the sum, so products with no January orders show 0 instead of NULL in the Jan column.", isCorrect: true },
            { id: 'c', text: "ELSE NULL and ELSE 0 produce identical results in SUM.", isCorrect: false },
            { id: 'd', text: "ELSE 0 is required for the CASE statement syntax to be valid.", isCorrect: false },
          ],
          explanation: "SUM ignores NULL values, so with ELSE NULL, a product with no January orders would show NULL in the Jan column. With ELSE 0, those rows contribute 0 instead — giving you a clean 0 in the result, which is usually what you want for reporting. Both are syntactically valid, but 0 vs NULL can affect downstream calculations and display."
        },
        {
          question: "You need to calculate the pass rate percentage per module. The formula is passed/total * 100. Which expression safely handles modules that have 0 total runs?",
          options: [
            { id: 'a', text: "passed / total * 100", isCorrect: false },
            { id: 'b', text: "passed / COUNT(*) * 100", isCorrect: false },
            { id: 'c', text: "ROUND(passed * 100.0 / NULLIF(total, 0), 1)", isCorrect: true },
            { id: 'd', text: "IF(total > 0, passed / total * 100, 0)", isCorrect: false },
          ],
          explanation: "NULLIF(total, 0) returns NULL when total is 0, making the division NULL instead of a divide-by-zero error. ROUND(..., 1) gives one decimal place. The IF approach also works but is less idiomatic for SQL. Direct division (passed / total) will throw an error if total is 0 in strict SQL mode. Multiplying by 100.0 (not 100) ensures decimal division, not integer."
        },
        {
          question: "You have a wide products table with columns q1_sales, q2_sales, q3_sales, q4_sales. You want to transform it into rows of (product_name, quarter, sales). Which technique achieves this in MySQL?",
          options: [
            { id: 'a', text: "UNPIVOT(sales FOR quarter IN (q1_sales, q2_sales, q3_sales, q4_sales))", isCorrect: false },
            { id: 'b', text: "SELECT name, 'Q1', q1_sales FROM products UNION ALL SELECT name, 'Q2', q2_sales FROM products UNION ALL SELECT name, 'Q3', q3_sales FROM products UNION ALL SELECT name, 'Q4', q4_sales FROM products", isCorrect: true },
            { id: 'c', text: "SELECT name, ROTATE(q1_sales, q2_sales, q3_sales, q4_sales) FROM products", isCorrect: false },
            { id: 'd', text: "GROUP BY quarter HAVING quarter IN ('Q1','Q2','Q3','Q4')", isCorrect: false },
          ],
          explanation: "MySQL has no native UNPIVOT keyword. The standard workaround is UNION ALL — each SELECT reads one column and labels it with a quarter name. UNION ALL is used (not UNION) to keep all rows including duplicates. UNPIVOT is SQL Server syntax. ROTATE and the GROUP BY option are not valid SQL."
        },
        {
          question: "You build a pivot report comparing pass rates between v2.1.0 and v2.2.0 per module. A module shows v2.1.0: 95%, v2.2.0: 78%. Your CASE verdict labels it '⚠️ REGRESSION'. A developer says 'this is wrong — a regression means tests that WERE passing are now failing'. Are they right?",
          options: [
            { id: 'a', text: "Yes — to detect true regressions, you must compare individual test case results (PASS→FAIL transitions), not aggregate pass rates.", isCorrect: true },
            { id: 'b', text: "No — a drop in pass rate percentage is the definition of a regression in all contexts.", isCorrect: false },
            { id: 'c', text: "No — the CASE verdict logic is identical to comparing individual test cases.", isCorrect: false },
            { id: 'd', text: "Yes — pass rates should never be used in QA reporting.", isCorrect: false },
          ],
          explanation: "The developer is correct. A pass rate drop from 95% to 78% could be caused by many things: new tests added (that are failing), flaky tests, environment issues — not necessarily regressions in existing tests. True regression detection requires joining test_name-level results between builds (test was PASS in v2.1.0 AND FAIL in v2.2.0). Aggregate pass rates are useful for trend reporting but insufficient for precise regression identification."
        },
      ]
    },
  ],
