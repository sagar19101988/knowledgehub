...
for (const mod of modulesData) {
  const safeMarkdown = mod.markdown
    .replace(/\\/g, '\\\\')
    .replace(/\`/g, '\\\`')
    .replace(/\\$/g, '\\$');

  injectedStr += ",\\n      {\\n";
  injectedStr += "        id: '" + mod.id + "',\\n";
  injectedStr += "        title: '" + mod.title + "',\\n";
  injectedStr += "        analogy: \\\"" + mod.analogy + "\\\",\\n";
  injectedStr += "        lessonMarkdown: \\`" + safeMarkdown + "\\`\\n";
  injectedStr += "      }";
}

let c = fs.readFileSync('src/data/analogies.ts', 'utf8');
const targetIndex = c.indexOf("id: 'destructuring',");

if (targetIndex > -1) {
    const endDestructuringTarget = c.indexOf("      }", targetIndex);
    if (endDestructuringTarget > -1) {
        const insertPosition = endDestructuringTarget + 7;
        c = c.substring(0, insertPosition) + injectedStr + c.substring(insertPosition);
